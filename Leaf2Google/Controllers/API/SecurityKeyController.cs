...
namespace Leaf2Google.Controllers.API
{
    ...
    public class SecurityKeyController : BaseAPIController
    {
        ...
        public SecurityKeyController(ICarSessionManager sessionManager, LeafContext leafContext, IOptions<Fido2Configuration> fido2Configuration)
            : base(sessionManager)
        {
        ...
        [HttpPost]
        public JsonResult MakeCredentialOptions([FromForm] string attType, [FromForm] string authType, [FromForm] bool requireResidentKey, [FromForm] string userVerification)
        {
            try
            {
                ...
                var existingKeys = _leafContext.SecurityKeys.Where(key => key.CredentialId.Length > 0).Select(key => new PublicKeyCredentialDescriptor(key.CredentialId)).ToList();
        ...
        public async Task<JsonResult> MakeCredential([FromBody] AuthenticatorAttestationRawResponse attestationResponse, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (await _leafContext.SecurityKeys.AnyAsync(key => key.CredentialId == args.CredentialId/*key.UserHandle == args.User.Id*/, cancellationToken))
                        return false;
            ...
            {
                return Json(new CredentialMakeResult(status: "error", errorMessage: e.Message, result: null));
            }
        }
        ...
        public async Task<JsonResult> MakeAssertionOptions([FromForm] string? username, [FromForm] string? userVerification)
        {
            try
            {
                ...
                var existingCredentials = credentials
                    ...
                    .Where(key => _leafContext.NissanLeafs.Any(car => car.NissanUsername == username && new Guid(key.UserId) == car.CarModelId))
        ...
        public async Task<JsonResult> MakeAssertion([FromBody] AuthenticatorAssertionRawResponse clientResponse, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var creds = securitykeys.Where(key => key.CredentialId.Length > 0).FirstOrDefault(key => new PublicKeyCredentialDescriptor(key.CredentialId).Id.SequenceEqual(clientResponse.Id)) ?? throw new Exception("Unknown credentials");
                ...
                {
                    var storedCreds = await _leafContext.SecurityKeys.FirstOrDefaultAsync(key => key.UserHandle == args.UserHandle, cancellationToken);
                ...
                if (key != null)
                {
                    ...
                    var car = await _leafContext.NissanLeafs.FirstOrDefaultAsync(car => car.CarModelId == new Guid(key.UserId));
